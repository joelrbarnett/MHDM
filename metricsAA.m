%Function metricsAA: returns metrics for AA-model multiscale decomposition
% F_orig is original image
% F_data is noisy image
% xkArray is the array of multiscale restored images
% T is blurring kernel
function [xk_f_norm2,rmse_final,stopCrit,snr]= metricsAA(F_orig,F_data,xkArray,T,numScales,tightFlag)
[m,n]=size(F_orig);
delta= sum(sum(F_data./imfilter(F_orig,T,'symmetric','same') + log(imfilter(F_orig,T,'symmetric','same')) - log(F_data) -ones(size(F_orig))));
...
for k=1:numScales
    ...
    D_f_data_Txk=sum(sum(F_data./imfilter(xkArray(:,:,k),T,'symmetric','same') + log(imfilter(xkArray(:,:,k),T,'symmetric','same')) - log(F_data)-ones(size(F_orig))  ));
    if tightFlag(1)==0 %stopping criteria for regular 
        stopCrit(k)= (D_f_data_Txk^2)/(delta^2);%ratio of bregman distances
    elseif tightFlag(1)==1 %stopping criteria for tight and refined versions
        alp = tightFlag(2)/(k)^(3/2);
        stopCrit(k)= (D_f_data_Txk^2+ alp*TV(log(xkArray(:,:,k))))/(delta^2);%ratio of bregman distances
    end
