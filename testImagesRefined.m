% 
% model: minimize int[lambda*( log(Tuxk) + f/Tuxk] + ak lambda TV(log(uxk))+ ||log(u)||_* over u. 
...
for j=1:length(images)-1 %loop over all images
    close all;
    %filenames for saving
    filePrefix="AA/"+fileNames(j)+"_noise_refined/";
...
%     F_data=double(F_data)+1;
    ...
    for k=1:numScales    
        ...
        uk=AAlog_blur_refined(F_data,xk,dt,lambda,alp,T, epsilon, maxIters);
